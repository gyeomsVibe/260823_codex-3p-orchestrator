...
# 4. 전체 단위 테스트 일괄 검증 (검증 실행 시 결과를 다시 확인)
python -m unittest discover -s tests -q
```

### 2. 자연어 실전 호출문

아래 문구는 대화 호스트에 **요청 의도**를 전달합니다. 문구 자체는 세 도구의 연결·회신·합의를 보장하지 않습니다. 실제 상태는 `python csc.py status`, 같은 작업 식별자의 `RESULT` 원문, 그리고 Codex의 최종 판정으로 확인합니다.

| 입력 문구 | 처리 계약 |
| --- | --- |
| `c3p 발동` | 유니코드 대소문자와 무관하게 C3P 실기동을 요청한다. |
| `MIA 씨3피 발동`, `MIA C3P 발동` | 유니코드 대소문자와 무관하게 MIA 전략 절차와 C3P 협업을 함께 요청한다. |
| `C3P 협의체와 검토해줘` | 세 도구의 실제 회신을 수집해 검토하도록 요청한다. 회신·정족수 증거가 없으면 합의로 표시하지 않는다. |
| `C3P 예산절약 모드`, `예산절약 모드 발동` | 정확히 일치할 때에만 예산절약 활성화를 요청한다. |
| `C3P 사용자부재 모드`, `사용자부재 모드 발동` | 정확히 일치할 때에만 사용자부재 감시 대기를 요청한다. |

지원되는 모든 자연어 호출문은 NFC 정규화, 공백 정리, 유니코드 대소문자 접기(Unicode case folding, 대문자·소문자를 같은 글자로 비교하는 처리)를 거쳐 비교합니다. 단, 글자 모양이 비슷한 다른 문자로 바꾸거나 문장 일부를 추정해 실행하지는 않습니다. `MIA … 발동`은 대화형 호스트에 협업을 요청하는 문구이고, `python csc.py trigger`는 로컬 프로세스를 시작하는 명시적 실기동·예산절약·사용자부재 문구만 받습니다.
