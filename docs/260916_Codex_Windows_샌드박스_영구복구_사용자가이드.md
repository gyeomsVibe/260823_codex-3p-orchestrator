...
현재는 공식 `:workspace` 권한 프로필을 기본값으로 사용한다. 일반 편집·테스트는 샌드박스 안에서 계속 수행하고, Git 쓰기는 승인 경계에서 검토한다. 예약 작업은 업데이트를 감지해 경계를 재검증하고 ACL은 읽기만 한다.
...
`tools/sync-codex-git-ignore.ps1`이 원본과 사본의 SHA-256 해시를 비교해 다를 때만 갱신한다. 유지관리 예약 작업이 이 사본도 복원한다.
...
`tools/maintain-codex-sandbox-after-updates.ps1`은 Codex 버전과 Windows 빌드를 마지막 정상 상태와 비교한다. 값이 바뀌면 위의 경계 검사 7개를 자동 실행한다. 버전이 같아도 24시간마다 전체 검사를 반복한다. 실패한 버전은 정상 상태로 저장하지 않으므로 30분 뒤 다시 검사한다.

`CodexSandboxUpdateMaintenance` 예약 작업은 로그온 시점과 30분 간격으로 실행된다. Git 경계 규칙과 Git ignore 사본은 자동 복원하지만, 사용자 `config.toml`과 Windows ACL은 덮어쓰지 않는다. 자세한 흐름은 [업데이트 후 자동검증 가이드](260916_Codex_Windows_업데이트후_샌드박스_자동검증_가이드.md)에 정리했다.
...
1. 유지관리 작업이 Codex 버전 또는 Windows 빌드 변경을 감지한다.
2. 자동 회귀 검사 7개를 실행한다.
3. 모두 통과한 경우에만 현재 버전을 마지막 정상 상태로 기록한다.
4. 실패하면 종료 코드 10을 남기고 30분 뒤 다시 검사한다.
5. ACL 감사 결과가 있어도 자동 삭제하지 않는다.
...
8. 저장소의 `.codex-sandbox-maintenance\state.json`에서 마지막 정상 버전을 확인한다.
...
| 예약 작업 대상 | 32개 경로 |
| 예약 작업의 ACL 변경 | 0건 |
| 업데이트 유지관리 예약 작업 | 로그온·30분 반복, exit 0 |
| 업데이트 상태 | Codex `0.154.0`, Windows `26100.8875`, healthy |
...
- Windows 또는 Codex 대규모 업데이트 뒤 ACL 구현이 어떻게 바뀔지

업데이트의 미래 동작 자체를 미리 보장할 수는 없다. 대신 버전 변경을 자동 감지하고 실제 경계 검사를 다시 실행한다. 검사에 실패한 버전은 정상으로 기록하지 않으며, 안전하게 관리할 수 있는 지원 파일만 복원한다.
