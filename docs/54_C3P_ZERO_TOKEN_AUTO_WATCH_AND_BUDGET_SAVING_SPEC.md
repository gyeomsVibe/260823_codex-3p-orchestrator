...
| **② 경제성·비용<br>(Economics)** | 토큰 소비 최소화<br>및 쿼터 보존 | **결함**: 3대 도구가 전체 텍스트를 감시하면 비용 폭증.<br>**해결책**: 무비용 파이썬 데몬 + 로컬 Ollama 오프로딩 + 3줄 요약 컨텍스트 실드로 외부 토큰의 대폭 절감을 설계 목표로 설정(RouteLLM 벤치마크 결과 참조 가설). |
...
| **④ 사용자 경험<br>(User-First)** | 사용 편의성 및<br>전문용어 3단 병기 | **결함**: 사용자가 매번 복잡한 런타임 옵션을 직접 지정해야 함.<br>**해결책**: `c3p 발동` 또는 각 도구의 시작 룰 파일(`AGENTS.md`, `CLAUDE.md`, `GEMINI.md`) 지침을 통해 1회 멱등 활성화 호출 지원. |
...
- **회귀 방어**: 전체 단위 테스트 100% 통과 유지 (문서 작성 시점 272개 -> 최신 309+개 전수 통과 확인).
