...
- **예산절약 모드 발동문**: `"C3P 예산절약 모드"`, `"C3P 예산절약 모드 발동"`, `"예산절약 모드"`, `"예산절약 모드 발동"`. 지원되는 자연어 호출문은 NFC 정규화·공백 정리·유니코드 대소문자 접기(Unicode case folding, 대문자·소문자를 같은 글자로 비교하는 처리)를 적용한다. 정확한 호출문만 `python csc.py trigger "<호출문>"`으로 기존 예산절약 활성화 경로를 호출하며, 설명 문장 속 포함은 발동이 아니다.
- **대소문자 계약**: 호출문의 유니코드 대소문자를 구분하지 않는다. 따라서 `c3p 발동`, `C3P 발동`, `C3p 발동`은 모두 같은 C3P 실기동 절차를 실행한다. 글자 모양이 비슷한 다른 문자를 같은 호출문으로 추정하지 않는다.
