...
- 지원되는 모든 자연어 호출문은 NFC 정규화·공백 정리·유니코드 대소문자 접기(Unicode case folding, 대문자·소문자를 같은 글자로 비교하는 처리)를 적용합니다. 글자 모양이 비슷한 다른 문자를 같은 호출문으로 추정하지 않습니다.
- 접두어 없는 **`c3p 발동`도 정식 호출문**이며 유니코드 대소문자를 구분하지 않습니다. `c3p 발동`, `C3P 발동`, `C3p 발동`은 같은 절차입니다.
- **C3P_CALL_PHRASE_CONTRACT_V1:** 지원되는 모든 자연어 호출문은 유니코드 대소문자를 구분하지 않습니다. `c3p 발동`은 실기동, `MIA C3P 발동`·`MIA 씨3피 발동`은 자연어 호스트의 MIA+C3P 요청, `C3P 협의체와 검토해줘`는 실제 회신 요구다. `c3p`, `C3P 협의체`, `C3P협의체` 단독은 모호하므로 실행하지 않는다.
- **예산절약 모드 발동문**은 `C3P 예산절약 모드`, `C3P 예산절약 모드 발동`, `예산절약 모드`, `예산절약 모드 발동`입니다. 유니코드 대소문자를 구분하지 않으며, 정확한 호출문만 `python csc.py trigger "<호출문>"`으로 활성화합니다. 설명 문장 속 단어만으로는 발동하지 않습니다.
- 위반은 `CALL_OUT` 대상입니다.
