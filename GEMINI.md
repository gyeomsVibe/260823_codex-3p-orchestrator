...
- **C3P_CALL_PHRASE_CONTRACT_V1:** `c3p 발동`은 실기동, `MIA C3P 발동`·`MIA 씨3피 발동`은 자연어 호스트의 MIA+C3P 요청, `C3P 협의체와 검토해줘`는 실제 회신 요구다. `c3p`, `C3P 협의체`, `C3P협의체` 단독은 모호하므로 실행하지 않는다.
- **예산절약 모드 발동문**은 `C3P 예산절약 모드`, `C3P 예산절약 모드 발동`, `예산절약 모드`, `예산절약 모드 발동`입니다. ASCII 영문은 대소문자를 구분하지 않으며, 정확한 호출문만 `python csc.py trigger "<호출문>"`으로 활성화합니다. 설명 문장 속 단어만으로는 발동하지 않습니다.
- 위반은 `CALL_OUT` 대상입니다.
