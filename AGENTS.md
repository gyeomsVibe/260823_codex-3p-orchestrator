...
- 접두어 없는 **`c3p 발동`도 정식 호출문**이며 유니코드 대소문자를 구분하지 않는다. `c3p 발동`, `C3P 발동`, `C3p 발동`은 같은 C3P 실기동 계약을 적용한다.
- **예산절약 모드 발동문**은 `C3P 예산절약 모드`, `C3P 예산절약 모드 발동`, `예산절약 모드`, `예산절약 모드 발동`이다. 유니코드 대소문자를 구분하지 않으며, 정확한 호출문만 `python csc.py trigger "<호출문>"`으로 예산절약 활성화를 요청한다. 설명 문장 속 단어만으로는 발동하지 않는다.
